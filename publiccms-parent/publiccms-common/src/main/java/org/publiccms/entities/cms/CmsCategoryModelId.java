 ...
 * @Embeddable 通过此注释可以在你的Entity中使用一般的java对象
 * 一个实体类要在多个不同的实体类中进行使用，而本身又不需要独立生成一个数据库表
 * CmsCategoryModel 通过@EmbeddedId 使用此对象 数据库中生成两个字段
 * CmsCategoryModel中@AttributeOverrides这两个注解是用来覆盖@Embeddable类中字段的属性的
 */
@Embeddable
