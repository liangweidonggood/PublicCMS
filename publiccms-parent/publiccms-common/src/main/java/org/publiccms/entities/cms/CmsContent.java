...
import org.publiccms.common.search.*;
 ...
 * 文章内容（此表要做索引）
 * @AnalyzerDef 采用Solr framework做分词
 *      name (Required) 定义该分词器的唯一字符串
 *      tokenizer (Required) 负责拆分输入流为单词或单字
 * @Analyzer
 *      definition 指向@AnalyzerDef标注中name属性定义的值
 *
 * @ClassBridge 用于一个实体对应的多个属性的联合索引，且该索引通常需要采用一种特殊的索引方式
 *
 * @FullTextFilterDefs 为了让Hibernate Search能够找到我们在产品类上添加的过滤器
 *
 * @Indexed 指定索引名称
 *
 */
